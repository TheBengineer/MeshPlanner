...
export default function App() {
    ...
    useEffect(() => {
        ...
        if (state.s.length > 0 || state.b) {
            ...
            if (state.vp) useStore.setState({
                viewport: {
                    latitude: state.vp.lat,
                    longitude: state.vp.lon,
                    zoom: state.vp.zoom
                }
            })
        ...
        <>
            ...
            <div className="top-bar">
                <div style={{display: 'flex', flexDirection: 'column', gap: 0}}>
                    <h2 data-testid="app-title" style={{
                        margin: 0,
                        fontSize: 15,
                        fontWeight: 600,
                        color: 'var(--text-h)',
                ...
                >
                        ...
                        <>
                            {guidedMode && (
                                <p style={{
                                    ...
                                    Set disaster area limits, add existing and required sites, then configure simulation
                                    parameters.
                                </p>
                            )}
                            ...
                            <div className="sidebar-section sidebar-section--padded">
                                ...
                                {guidedMode ? <h3 id="step-existing-required"
                                                  style={{margin: '0 0 8px', fontSize: 13, fontWeight: 600}}>Existing
                                        and Required Sites</h3> :
                                    <div className="section-label">Existing and Required Sites</div>}
                                ...
                                <CriticalInfraImport/>
                                <button type="button" onClick={() => useStore.getState().triggerCenterOnSite()} style={{
                                    width: '100%',
                                    padding: '4px 8px',
                                    marginTop: 4,
                                    marginBottom: 4,
                    ...
                    {guidedMode && activeTab === 'plan' && (
                        <p style={{
                            ...
                            Run the mesh planner to automatically find optimal hilltop relay locations and build a
                    ...
                    {activeTab === 'setup' && (
                        <>
                            {guidedMode && <h3 id="step-params" style={{
                                margin: '8px 12px 0',
                                fontSize: 13,
                                fontWeight: 600
                            }}>Params</h3>}
                            ...
                            {(() => {
                                return (
                                    <div style={{borderTop: '1px solid var(--border)', marginTop: 8}}>
                                        <div role="button" tabIndex={0} onClick={() => setAdvOpen(!advOpen)}
                                             onKeyDown={(e) => {
                                                 if (e.key === 'Enter' || e.key === ' ') {
                                                     e.preventDefault();
                                                     setAdvOpen(!advOpen)
                                                 }
                                             }} aria-expanded={advOpen} style={{
                                            display: 'flex',
                                            alignItems: 'center',
                                            gap: 6,
                                            padding: '8px 12px',
                                        ...
                                        {advOpen && (
                                            <div style={{
                                                ...
                                                <button type="button" onClick={() => setShowTerrain(!showTerrain)}
                                                        aria-pressed={showTerrain} style={{
                                                    width: '100%',
                                                    padding: '6px 10px',
                                                    fontSize: 12,
                                                    fontWeight: 600,
                                                ...
                                                <button type="button" onClick={() => setShowBuildings(!showBuildings)}
                                                        aria-pressed={showBuildings} style={{
                                                    width: '100%',
                                                    padding: '6px 10px',
                                                    fontSize: 12,
                                                    fontWeight: 600,
                                                ...
                                                <label style={{
                                                    ...
                                                    <input type="checkbox"
                                                           checked={useStore.getState().coverageParams.debugTerrain ?? false}
                                                           onChange={e => useStore.getState().updateCoverageParams({debugTerrain: e.target.checked})}
                                                           aria-label="Debug terrain mode"/>
                                                    Debug Terrain
                                                </label>
                                            </div>
                                        )}
                        ...
                        <>
                            {guidedMode && (
                                <p style={{
                                    ...
                                    Review coverage metrics, compare optimization results, and export your deployment
                                    plan.
                                </p>
                            )}
                            <ExportPanel/>
